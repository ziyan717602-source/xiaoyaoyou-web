...
const RoomPage: React.FC = () => {
  ...
  // If we have a roomId from URL but no room state, fetch it
  useEffect(() => {
    if (websocket.state.isConnected && roomId && !room.currentRoom) {
      websocket.send({ type: 'get_room', payload: { roomId } });
    }
  ...
  const hasEmptySlots = players.length < (room.currentRoom?.maxPlayers ?? 6);
    ...
    <div className="room-page">
      ...
      <div className="room-content">
        ...
        <div className="room-info">
          ...
          <div className="room-info-item">
            ...
            <span className="room-info-value">{players.length} / {room.currentRoom?.maxPlayers ?? 6}</span>
          </div>
        </div>
      </div>
      ...
      <div className="room-footer">
        {isHost && hasEmptySlots && (
          <button className="btn btn-secondary" onClick={handleAddAI}>
            添加AI玩家
          </button>
        )}
        {isHost && (
