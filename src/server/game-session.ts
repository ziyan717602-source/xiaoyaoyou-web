...
import type { GameState, DecisionResponse } from '../shared/network/protocol';
import { InputManager } from '../shared/game/input-manager';
import { ActorRegistry } from './actors/actor-registry';
import { HumanActor } from './actors/human-actor';
...
export class GameSession {
  ...
  constructor(
    ...
    // Set up InputManager decision_request callback
    this.inputManager.onDecisionRequest((request) => {
      // Send decision_request to all recipients
    ...
    // Register actors for all players (AI actors created in start())
    this.registerActors();
  ...
  async start(): Promise<GameResult> {
    ...
    try {
      ...
      // Register AI actors after game is initialized
      this.registerAIActors();
  ...
  /**
   * Handle a player's decision response (M2).
   * @returns true if the response was consumed, false if no pending decision.
   */
  handleDecisionResponse(uid: number, response: DecisionResponse): boolean {
    const result = this.inputManager.submitDecision(response);
    if (!result.accepted) {
      // Send rejection back to the player
      const connId = this.findConnectionIdByUid(uid);
      if (connId) {
        this.connectionManager.send(connId, {
  ...
  /**
   * Register HumanActor and AIActor for all players.
   */
  private registerActors(): void {
    // Register HumanActors for connected network players
    for (const [connId, mapping] of this.connectionToPlayer) {
      if (mapping.roomId === this.room.id) {
        ...
        this.actorRegistry.register(actor);
      }
    }
  }

  /**
   * Register AIActor for AI players (called after game.initialize()).
   */
  private registerAIActors(): void {
    for (const [uid] of this.game.getPlayers()) {
      if (this.actorRegistry.get(uid)) continue; // Already registered as HumanActor
      const aiPlayer = this.game.getAIPlayers().get(uid);
      if (aiPlayer) {
        // AIActor wraps the existing AIPlayer's strategy
        const strategy = this.createAIStrategies()[uid - 1];
        if (strategy) {
          const actor = new AIActor(uid, strategy, this.game.getBoard());
          this.actorRegistry.register(actor);
        }
      }
    }
