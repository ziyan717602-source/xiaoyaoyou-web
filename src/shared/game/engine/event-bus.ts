...
/**
 * Events that MUST use emitAsync() to avoid phase-skip bugs.
 * Used in development guard to warn when emit() is called for rule events.
 */
...
export class EventBus {
  ...
  emit(event: string, data?: unknown): EventResult {
    // Dev guard: warn when rule events use synchronous emit
