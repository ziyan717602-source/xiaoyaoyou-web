...
export class GLoop {
  ...
  private async handleDefaultCommand(cmdType: string, args: string[], cmd: string): Promise<void> {
    switch (cmdType) {
      ...
      case 'G0QR': await this.handleQuarterReset(args); break;
      case 'G0HC': await this.handleHarvestPet(args); break;
      ...
      case 'G0HT': await this.handleG0HT(args); break;
      ...
      case 'G0ZJ': await this.handleEquipSlotVariation(args); break;
    }
  }
   ...
   */
  private async handleQuarterReset(args: string[]): Promise<void> {
    ...
    if (excess > 0) {
      await this.raiseGMessage(`G0DH,${uid},1,${excess}`);
    }
  }
   ...
   */
  private async handleHarvestPet(args: string[]): Promise<void> {
    ...
    await this.raiseGMessage(`G0HD,${farmer},0,${trophy},${petList}`);
  }
   ...
   */
  private async handleG0HT(args: string[]): Promise<void> {
    const who = parseInt(args[1], 10);
    const n = parseInt(args[2], 10);
    if (isNaN(who) || isNaN(n) || n <= 0) return;
    await this.raiseGMessage(`G0DH,${who},0,${n}`);
  }
   ...
   */
  private async handleEquipSlotVariation(args: string[]): Promise<void> {
      ...
      if ((player.exMask & mask) !== 0) {
        ...
        if (player.exEquip !== 0) {
          ...
          if (orgEquip === 0) {
            ...
            await this.raiseGMessage(`G0QZ,${who},${player.exEquip}`);
            player.exEquip = 0;
          }
        }
      } else {
        ...
        const orgEquip = slotType === 0 ? player.weapon : slotType === 1 ? player.armor : player.trove;
        if (orgEquip !== 0) {
          await this.raiseGMessage(`G0QZ,${who},${orgEquip}`);
        }
      }
    }
  }
