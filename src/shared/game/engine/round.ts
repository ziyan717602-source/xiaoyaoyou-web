...
export class RoundManager {
  ...
  private async onRoundInit(state: RoundState): Promise<void> {
    ...
    if (rounder && rounder.immobilized) {
      ...
      await this.eventBus.emitAsync('raise:gmessage', { cmd: `G0QR,${state.rounderUid}` });
      await this.eventBus.emitAsync('raise:gmessage', { cmd: `G0DS,${state.rounderUid},1` });
    }
  ...
  private async onEV(state: RoundState): Promise<void> {
    ...
    if (this.inputCallback) {
      ...
      if (decision === '2') {
        ...
        await this.eventBus.emitAsync('raise:gmessage', { cmd: `G1EV,${rounder.uid},0` });
      } else {
        // Skip event
        if (this.messageHandler) {
      ...
      if (playableCards.length > 0) {
        ...
        if (cardChoice && cardChoice !== '/' && cardChoice !== '0' && cardChoice !== '') {
          ...
          for (const code of codes) {
            ...
            await this.eventBus.emitAsync('raise:gmessage', { cmd: `G0CC,${rounder.uid},${cardId},0` });
  ...
  private async onZW(state: RoundState): Promise<void> {
    ...
    if (state.isFight) {
      await this.eventBus.emitAsync('raise:gmessage', { cmd: 'G1SG,0' });
    }
  ...
  private async onZM(state: RoundState): Promise<void> {
    ...
    if (this.board.mon1From === 0 && this.board.monster1 === 0) {
      if (this.board.monPiles.count > 0) {
        ...
        await this.eventBus.emitAsync('raise:gmessage', { cmd: 'G1WJ,0' });
        return;
      }
    }
   ...
   */
  private async applyZPEffect(code: string, player: Player): Promise<void> {
    switch (code) {
      case 'ZP01':
      ...
      case 'ZP02':
        ...
        await this.eventBus.emitAsync('raise:gmessage', {
  ...
  private async onVS(state: RoundState): Promise<void> {
    ...
    if (this.board.isBattleWin) {
      ...
      if (this.board.monster1 !== 0) {
        await this.eventBus.emitAsync('raise:gmessage', { cmd: `G1GE,W,${this.board.monster1}` });
      }
      const hasMonster2 = this.board.monster2 !== 0 && NMBLib.isMonster(this.board.monster2);
      if (hasMonster2) {
        await this.eventBus.emitAsync('raise:gmessage', { cmd: `G1GE,W,${this.board.monster2}` });
      }
      ...
      if (this.board.monster1 !== 0 && this.board.mon1Catchable) {
        await this.eventBus.emitAsync('raise:gmessage', { cmd: `G0HC,0,${state.rounderUid},${this.board.mon1From},${mask},${this.board.monster1}` });
      }
      if (hasMonster2 && this.board.mon2Catchable) {
        await this.eventBus.emitAsync('raise:gmessage', { cmd: `G0HC,0,${state.rounderUid},0,${mask},${this.board.monster2}` });
      }
    } else {
      // Lose: raise G1GE,L for each monster
      if (this.board.monster1 !== 0) {
        await this.eventBus.emitAsync('raise:gmessage', { cmd: `G1GE,L,${this.board.monster1}` });
      }
      if (this.board.monster2 !== 0 && NMBLib.isMonster(this.board.monster2)) {
        await this.eventBus.emitAsync('raise:gmessage', { cmd: `G1GE,L,${this.board.monster2}` });
      }
    }
