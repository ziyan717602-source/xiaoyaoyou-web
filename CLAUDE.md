...
│   ├── client/               # React 前端
│   │   ├── components/       # UI 组件
│   │   │   ├── game/         # 游戏组件（HandArea, BattleArea, InputController, SkillPanel 等）
│   │   │   └── layout/       # 布局组件
│   │   ├── pages/            # 页面（Home, Room, Game, Result）
│   │   ├── hooks/            # React hooks（useGameState, useRoom, useWebSocket）
...
服务端发送格式化字符串（如 `T1(p1p2p3)##决定##取消`），客户端使用 `src/client/utils/format-parser.ts` 解析为结构化段（Segment），再由 `InputController` 渲染对应的交互 UI。
...
| 编号 | 问题 | 状态 | 参考 |
|------|------|------|------|
| E-01 | runStage() 优先级派发（技能触发系统） | ✅ 已完成 | `src/shared/game/engine/g-loop.ts` |
| E-02 | g0ht 监听器（战后摸牌阶段） | ✅ 已完成 | `src/shared/game/engine/round.ts` + `g-loop.ts` |
| BUG-01 | 选人重复：Game.selectHeroes() 重复运行 | ✅ 已修复 | `game.ts` heroesSelected 标志 |
| BUG-02 | ZD阶段只处理ZP牌，不支持战牌/技能 | ✅ 已修复 | `round.ts` 改为 runStage() |
| BUG-03 | ZC阶段缺少技能触发 | ✅ 已修复 | `round.ts` 添加 run:stage |
| ROOT-1 | `run:stage` 未连接到 GLoop.runStage() | ✅ 已修复 | `game.ts` 添加事件监听 |
| ROOT-2 | skill occur `#` 通配符未展开 | ✅ 已修复 | `skill-registry.ts` findHandlers |
| ROOT-3 | 事件牌信息未发送到客户端 | ✅ 已修复 | `g-loop.ts` + `protocol.ts` + `game-session.ts` |
| FIX-4 | 洞冥宝镜无触发点 | ✅ 已修复 | `round.ts` onZW raise G1SG |
| 待查 | 选人列表包含 NPC 角色 | 🟡 中 | listAllSeleable 过滤逻辑 |
| 待查 | 妨碍者选择应发给敌方全队 | 🟡 中 | onZW opponent 逻辑 |
| 待查 | 洞冥宝镜偷看机制 | 🟡 中 | ZM 之前的偷看 |
