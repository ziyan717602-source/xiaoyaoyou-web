    ...
    return {
      ...
      action: async (player, _type, _fuse, _argst) => {
        ...
        if (!ai.startsWith('/')) {
          ...
          const fromPlayer = this.board.garden.get(from);
          if (fromPlayer) {
            await this.asyncInput(
              player.uid,
              `#获得的,C1(${fromPlayer.tux.map(() => 'p0').join('')})`,
              'JP01',
              '0',
            );
          }
    ...
    return {
      ...
      action: async (player, type, fuse, _argst) => {
        if (type === 0) {
          ...
          this.targetPlayer(player.uid, to);
          const toPlayer = this.board.garden.get(to);
          if (toPlayer) this.harm(player, toPlayer, 2, FiveElement.THUNDER, HPEvoMask.FROM_JP);
        } else if (type === 1) {
      ...
      action: async (player, type, _fuse, _args) => {
        if (type === 0) {
          ...
          if (invs.includes(tg)) {
            ...
            const tgPlayer = this.board.garden.get(tg);
            if (tgPlayer) this.cure(player, tgPlayer, 2);
          }
    ...
    return {
      ...
      action: (player, _type, _fuse, _argst) => {
        ...
        const pys = list
          ...
          .filter((p): p is Player => p != null && p.isAlive && p.team === player.team);
        if (pys.length > 0) {
      ...
      action: async (player, type, fuse, _argst) => {
        if (type === 0) {
          ...
          for (const part of parts) {
            ...
            const n = parseInt(pParts[3], 10);
            const mask = parseInt(pParts[4], 10) || 0;
            const whoPlayer = this.board.garden.get(who);
            if (n > 1 && whoPlayer &&
              whoPlayer.isTared &&
