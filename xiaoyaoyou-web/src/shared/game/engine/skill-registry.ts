...
export class SkillRegistry {
  ...
  registerSkill(skill: Skill | Bless, handler: EventHandler): void {
    ...
    for (let i = 0; i < skill.occurs.length; i++) {
      ...
      // Wrap handler: EventBus passes data object, but ActionDelegate expects (player, type, fuse, argst)
      const wrappedHandler = async (data: unknown): Promise<unknown> => {
        const d = data as Record<string, unknown> | undefined;
        const player = d?.player ?? d?.board?.rounder ?? null;
        const type = Number(d?.type ?? d?.inType ?? 0);
        const fuse = String(d?.fuse ?? d?.cmd ?? '');
        const argst = String(d?.args ?? d?.argst ?? '');
        if (player && typeof handler === 'function') {
          try {
            return await handler(player, type, fuse, argst);
          } catch (e) {
            console.error(`[SkillRegistry] Handler error for skill at event:`, e);
          }
        }
        return undefined;
      };
      ...
      this.addToSk02(oc, skt);
      this.eventBus.on(oc, wrappedHandler, skt.priorty, `skill:${code}`);
      this.addRegisteredHandler(oc, wrappedHandler, skt, 'skill', code);
    }
  }
  ...
  findHandlers(eventKey: string): SkTriple[] {
    ...
    // Also check wildcard-expanded keys: R#ST -> match R1ST, R2ST, etc.
    // The '#' in occur strings like 'R#ST' should match any UID digit
    if (eventKey.length >= 3 && eventKey[0] === 'R') {
      const wildcardKey = eventKey[0] + '#' + eventKey.substring(2);
      if (wildcardKey !== eventKey) {
        const wildcardList = this.sk02.get(wildcardKey);
        if (wildcardList) {
          result.push(...wildcardList);
        }
      }
    }
