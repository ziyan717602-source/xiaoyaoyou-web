...
export class ConnectionManager {
   ...
   * Falls back to any connection in the room if exact match fails
   * (handles reconnection timing where the new connection hasn't been mapped yet).
   */
  findConnectionByPlayer(roomId: string, playerName: string): Connection | undefined {
    // Exact match first
    const exact = Array.from(this.connections.values()).find(
      (c) => c.roomId === roomId && c.playerName === playerName,
    );
    if (exact) return exact;

    // Fallback: find any live connection in the room
    // This handles the case where a player reconnected and the new connection
    // hasn't been mapped with playerName yet
    const fallback = Array.from(this.connections.values()).find(
      (c) => c.roomId === roomId && c.ws.readyState === 1, // WebSocket.OPEN = 1
    );
    return fallback;
  }
